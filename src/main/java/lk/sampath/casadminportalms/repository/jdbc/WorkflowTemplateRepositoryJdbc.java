...
public class WorkflowTemplateRepositoryJdbc {
    ...
    public List<WorkflowTemplateDTO> getWorkflowTemplate(WorkflowTemplateRequest request) {
        ...
        if (StringUtils.isNotEmpty(request.getWorkFlowTemplateName())) {
            sql.append("AND upper(wft.WORK_FLOW_TEMPLATE_NAME) LIKE '%").append(request.getWorkFlowTemplateName().toUpperCase()).append("%' ");
        }
        if (StringUtils.isNotEmpty(request.getCode())) {
            sql.append("AND upper(wft.CODE) LIKE '%").append(request.getCode().toUpperCase()).append("%' ");
        }
        if (StringUtils.isNotEmpty(request.getDescription())) {
            sql.append("AND upper(wft.DESCRIPTION) LIKE '%").append(request.getDescription().toUpperCase()).append("%' ");
        }
        if (request.getApproveStatusList() != null && !request.getApproveStatusList().isEmpty()) {
            sql.append("AND wft.APPROVE_STATUS IN (").append(buildSQLINQuery(request.getApproveStatusList())).append(") ");
        }
        ...
        return namedParameterJdbcTemplate.query(sql.toString(), params, new RowMapper<WorkflowTemplateDTO>() {
            @Nullable
            @Override
