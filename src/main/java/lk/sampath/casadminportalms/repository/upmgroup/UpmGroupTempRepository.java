...
@Repository
public interface UpmGroupTempRepository extends JpaRepository<UpmGroupTemp, Integer>, QuerydslPredicateExecutor<UpmGroupTemp> {

    @Query(value = "SELECT SEQ_T_UPM_GROUP.NEXTVAL FROM DUAL", nativeQuery = true)
    Integer getCurrentSequenceValue();

    @Query( value = "SELECT * FROM t_upm_group_temp WHERE approve_status NOT IN ('APPROVED')", nativeQuery = true)
