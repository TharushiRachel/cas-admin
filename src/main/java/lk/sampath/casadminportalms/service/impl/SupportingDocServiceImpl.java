...
public class SupportingDocServiceImpl implements SupportingDocService {
    ...
    @Override
    public ResponseEntity<StandardResponse<SupportingDocDTO>> findSupportingDocTempById(Integer supportingDocID) throws ApiRequestException{
        SupportingDocTemp supportingDocTemp = supportingDocTempRepository.findById(supportingDocID)
                .orElseThrow(() -> new ApiRequestException(SUPPORTING_DOC_WITH + supportingDocID + DOES_NOT_EXISTS));
    ...
    public ResponseEntity<StandardResponse<SupportingDocDTO>> findSupportingDocById(Integer supportingDocID) throws ApiRequestException{
        SupportingDoc supportingDoc = supportingDocRepository.findById(supportingDocID)
                .orElseThrow(() -> new ApiRequestException(SUPPORTING_DOC_WITH + supportingDocID + DOES_NOT_EXISTS));
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), supportingDoc);
        return ResponseEntity.ok().body(response);
    }
    ...
    public ResponseEntity<StandardResponse<SupportingDocDTO>> saveSupportingDocTemp (SupportingDocDTO supportingDocDTO) throws ApiRequestException {
        ...
        booleanBuilder.and(QSupportingDocTemp.supportingDocTemp.documentName.eq(supportingDocDTO.getDocumentName()));
        List<SupportingDocTemp> supportingDocTemps = (List<SupportingDocTemp>) supportingDocTempRepository.findAll(booleanBuilder);

        validateSupportingDocNameUniqueness(supportingDocDTO.getDocumentName(), null);

        if(supportingDocTemps.isEmpty()){
            supportingDocTempSave.setSupportingDocID(supportingDocTempRepository.getCurrentSequenceValue());
        ...
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), supportingDocTempSave);
        return ResponseEntity.ok().body(response);
    ...
    public ResponseEntity<StandardResponse<SupportingDocDTO>> approveRejectSupportingDoc (ApproveRejectRQ approveRejectRQ) throws ApiRequestException {
        ...
        SupportingDocTemp supportingDocTemp = supportingDocTempRepository.findById(approveRejectRQ.getApproveRejectDataID())
                .orElseThrow(() -> { throw new ApiRequestException(SUPPORTING_DOC_WITH + approveRejectRQ.getApproveRejectDataID() + DOES_NOT_EXISTS);
                });
    ...
    private ResponseEntity<StandardResponse<SupportingDocDTO>> handleApproval (SupportingDocTemp supportingDocTemp, SupportingDoc supportingDoc) {
        SupportingDoc savedSupportingDoc;

        if (supportingDoc != null && supportingDoc.getSupportingDocID().equals(supportingDocTemp.getSupportingDocID())) {
        ...
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), savedSupportingDoc);
        return ResponseEntity.ok().body(response);
    }
    ...
    private SupportingDoc updateSupportingDocToMaster ( SupportingDocTemp supportingDocTemp, SupportingDoc existingSupportingDoc) {

        SupportingDoc supportingDoc = (existingSupportingDoc != null) ? existingSupportingDoc : new SupportingDoc();
    ...
    private SupportingDoc mapSupportingDoc(SupportingDocTemp supportingDocTemp, SupportingDoc existingSupportingDoc) {
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
    public ResponseEntity<StandardResponse<SupportingDocDTO>> updateSupportingDocTemp(Integer supportingDocID, SupportingDocDTO supportingDocDTO) throws ApiRequestException {

        SupportingDocTemp supportingDocTemp = supportingDocTempRepository.findById(supportingDocID).orElseThrow(() -> {
        ...
        BooleanBuilder booleanBuilder = new BooleanBuilder();
        booleanBuilder.and(QSupportingDocTemp.supportingDocTemp.documentName.eq(supportingDocDTO.getDocumentName()));
        List<SupportingDocTemp> supportingDocTempList = (List<SupportingDocTemp>) supportingDocTempRepository.findAll(booleanBuilder);

        if ( supportingDocDTO.getDocumentName() == null || supportingDocDTO.getDocumentName().trim().isEmpty()) {
            throw new ApiRequestException(EMPTY_NULL);
        }

        validateSupportingDocNameUniqueness(supportingDocDTO.getDocumentName(), supportingDocID);

        boolean existingTempList = supportingDocTempList.stream()
                        .anyMatch(temp -> temp.getDocumentName().equals(supportingDocDTO.getDocumentName()));

        if(existingTempList && !supportingDocTemp.getDocumentName().equals(supportingDocDTO.getDocumentName())){
            throw new ApiRequestException(SUPPORTING_DOC_TEMP_WITH + supportingDocDTO.getDocumentName() + ALREADY_EXISTS);
        }
    ...
    private void validateSupportingDocNameUniqueness(String documentName, Integer supportingDocID) throws ApiRequestException {
        ...
        if (existsInTemp || existsInMaster) {
            throw new ApiRequestException("Document name '" + documentName + "' already exists in the system.");
        }
    }


    private SupportingDocTemp mapToSupportingDocTemp (SupportingDoc supportingDoc, SupportingDocDTO supportingDocDTO) {
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
    public ResponseEntity<StandardResponse<Void>> deleteSupportingDocTemp(Integer supportingDocID) throws ApiRequestException {
        supportingDocTempRepository.deleteById(supportingDocID);
        StandardResponse<Void> response = new StandardResponse<>(ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), supportingDocID);
        return ResponseEntity.ok().body(response);
    }
