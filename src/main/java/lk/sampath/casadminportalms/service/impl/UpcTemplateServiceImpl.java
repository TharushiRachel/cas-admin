...
public class UpcTemplateServiceImpl implements UpcTemplateService {
  ...
  public UpcTemplateServiceImpl(
      UpcTemplateTempRepository upcTemplateTempRepository,
      UpcTemplateDataTempRepository upcTemplateDataTempRepository,
      UpcSectionRepository upcSectionRepository,
      UpcTemplateRepository upcTemplateRepository,
      UpcTemplateDataRepository upcTemplateDataRepository,
      UpcTemplateAudRepository upcTemplateAudRepository,
      UpcTemplateDataAudRepository upcTemplateDataAudRepository) {
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Object>> findUpcTemplateTempById(Integer upcTemplateID)
      throws ApiRequestException {
    log.info("START: Find Upc Template Temp By ID with UPC Template ID: {}", upcTemplateID);
    UpcTemplateTemp upcTemplateTemp =
        upcTemplateTempRepository
            .findById(upcTemplateID)
            .orElseThrow(
    ...
    StandardResponse<Object> response =
        new StandardResponse<>(
            ...
            upcTemplateDTO);
    log.info(
        "END: Find Upc Template Temp By ID: {} with Response: {}",
        upcTemplateID,
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Object>> findApprovedUpcTemplateById(Integer upcTemplateID)
      throws ApiRequestException {
    log.info("START: Find Approved Upc Template By ID with UPC Template ID: {}", upcTemplateID);
    UpcTemplate upcTemplate =
        upcTemplateRepository
            .findById(upcTemplateID)
            .orElseThrow(
                () -> new ApiRequestException(UPC_TEMPLATE_WITH + upcTemplateID + DOES_NOT_EXIST));
    ...
    StandardResponse<Object> response =
        new StandardResponse<>(
            ...
            upcTemplateDTO);
    log.info(
        "END: Find Approved Upc Template By ID with UPC Template ID: {}. Response: {}",
        upcTemplateID,
        response.getResponse());
    return ResponseEntity.ok().body(response);
  }
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Object>> saveUpcTemplate(UpcTemplateDTO upcTemplateDTO)
      throws ApiRequestException {
    ...
    List<UpcTemplateTemp> upcTemplateTempList =
        (List<UpcTemplateTemp>) upcTemplateTempRepository.findAll(booleanBuilder);
    log.info(
        "Checked for Existing Template. Found {} existing templates with name: {}",
        upcTemplateTempList.size(),
        upcTemplateDTO.getTemplateName());
    ...
    if (upcTemplateTempList.isEmpty()) {
      ...
      if (upcTemplateDTO.getUpcTemplateDataDTOList() != null) {
        List<UpcTemplateDataTemp> upcTemplateDataTempList =
            saveOrUpdateUpcTemplateData(upcTemplateDTO, upcTemplateTemp);
    ...
    StandardResponse<Object> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), upcTemplate);
    log.info("END: Save Upc Template successful. Response: {}", response.getResponse());
    return ResponseEntity.ok().body(response);
  }

  public List<UpcTemplateDataTemp> saveOrUpdateUpcTemplateData(
      UpcTemplateDTO upcTemplateDTO, UpcTemplateTemp upcTemplateTemp) {
    ...
    if (upcTemplateDTO.getIsModified().equals(AppsConstants.YesNo.Y)) {
      List<UpcTemplateDataTemp> existingDataTemps =
          upcTemplateDataTempRepository.findByUpcTemplateTempUpcTemplateID(
              upcTemplateTemp.getUpcTemplateID());
      ...
      for (UpcTemplateDataDTO upcTemplateDataDTO : upcTemplateDTO.getUpcTemplateDataDTOList()) {
        UpcTemplateDataTemp upcTemplateDataTemp =
            createUpcTemplateDataTemp(upcTemplateTemp, upcTemplateDataDTO);
  ...
  private UpcTemplateDataTemp createUpcTemplateDataTemp(
      UpcTemplateTemp upcTemplateTemp, UpcTemplateDataDTO upcTemplateDataDTO) {
    UpcSection upcSection =
        upcSectionRepository
            .findById(upcTemplateDataDTO.getUpcSectionID())
            .orElseThrow(
                () ->
                    new ApiRequestException(
                        "Upc section with ID "
                            + upcTemplateDataDTO.getUpcSectionID()
                            + " does not exist."));

    UpcTemplateDataTemp upcTemplateDataTemp = new UpcTemplateDataTemp();
    upcTemplateDataTemp.setUpcTemplateDataID(
        upcTemplateDataTempRepository.getCurrentSequenceValue());
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Object>> updateUpcTemplateTemp(
    ...
    if (upcTemplateDTO.getUpcTemplateDataDTOList() != null
        && upcTemplateDTO.getIsModified().equals(AppsConstants.YesNo.Y)) {
      List<UpcTemplateDataTemp> upcTemplateDataTempList =
          saveOrUpdateUpcTemplateData(upcTemplateDTO, upcTemplateTemp);
      upcTemplateTemp.setUpcTemplateDataTempList(upcTemplateDataTempList);
    }
    ...
    UpcTemplateTemp upcTemplateTemp =
        upcTemplateTempRepository
            ...
            .orElseThrow(
                () ->
                    new ApiRequestException(
                        UPC_TEMPLATE_WITH_ID
                            + approveRejectRQ.getApproveRejectDataID()
                            + DOES_NOT_EXIST));
    log.info("Fetched UpcTemplateTemp for approval/rejection: {}", upcTemplateTemp);

    Optional<UpcTemplate> optionalUpcTemplate =
        upcTemplateRepository.findById(upcTemplateTemp.getUpcTemplateID());
  ...
  private ResponseEntity<StandardResponse<Object>> handleApproval(
      UpcTemplateTemp temp, UpcTemplate existingTemplate) {
    UpcTemplate savedTemplate;

    if (existingTemplate != null
        && existingTemplate.getUpcTemplateID().equals(temp.getUpcTemplateID())) {
  ...
  private UpcTemplate mapUpcTemplate(
      UpcTemplateTemp upcTemplateTemp, UpcTemplate existingUpcTemplate) {

    UpcTemplate upcTemplate =
        (existingUpcTemplate != null) ? existingUpcTemplate : new UpcTemplate();
    ...
    if (upcTemplateTemp.getUpcTemplateDataTempList() != null) {
      List<UpcTemplateData> upcTemplateDataList =
          saveOrUpdateMasterTemplateData(upcTemplateTemp, upcTemplate);
  ...
  public UpcTemplate updateUpcTemplateToMaster(
      UpcTemplateTemp upcTemplateTemp, UpcTemplate upcTemplate) {
    ...
    if (upcTemplateTemp.getUpcTemplateDataTempList() != null) {
      List<UpcTemplateData> updatedTemplateDataList =
          saveOrUpdateMasterTemplateData(upcTemplateTemp, updatedUpcTemplate);
  ...
  private void saveUpcTemplateDataAudit(List<UpcTemplateDataTemp> dataTempList) {
    ...
    List<UpcTemplateDataAud> audits =
        dataTempList.stream()
            .map(
                temp -> {
  ...
  public List<UpcTemplateData> saveOrUpdateMasterTemplateData(
      UpcTemplateTemp temp, UpcTemplate master) {

    if (temp == null || master == null || temp.getUpcTemplateDataTempList() == null) {
      log.info(
          "END: Input is invalid. Temp: {}, Master: {}, Template Data Temp List is null.",
          temp,
          master);
    ...
    List<UpcTemplateData> upcTemplateDataList =
        upcTemplateDataRepository.findByUpcTemplateUpcTemplateID(master.getUpcTemplateID());
    if (!upcTemplateDataList.isEmpty()) {
      upcTemplateDataRepository.deleteAll(upcTemplateDataList);
    }

    List<UpcTemplateData> masterData =
        temp.getUpcTemplateDataTempList().stream()
            .map(
                tempData -> {
    ...
    log.info(
        "END: Save/Update Master Template Data for Master Template ID: {}",
        master.getUpcTemplateID());
  ...
  public ResponseEntity<StandardResponse<Object>> updateApprovedUpcTemplate(
    ...
    if (!upcTemplate.getTemplateName().equals(upcTemplateDTO.getTemplateName())) {
      validateTemplateNameUniqueness(upcTemplateDTO.getTemplateName(), upcTemplateID);
    } else if (upcTemplateDTO.getTemplateName() == null
        || upcTemplateDTO.getTemplateName().trim().isEmpty()) {
    ...
    if (upcTemplateDTO.getUpcTemplateDataDTOList() != null) {
      List<UpcTemplateDataTemp> upcTemplateDataTempList =
          updateUpcTemplateTempFromMaster(upcTemplateDTO, upcTemplateTemp);
      upcTemplateTemp.setUpcTemplateDataTempList(upcTemplateDataTempList);
    }
  ...
  private void validateTemplateNameUniqueness(String templateName, Integer upcTemplateID)
      throws ApiRequestException {
    ...
    if (existsInTemp || existsInMaster) {
      throw new ApiRequestException(
          "User name '" + templateName + "' already exists in the system.");
    }
  }

  private UpcTemplateTemp mapToUpcTemplateTemp(
      UpcTemplate upcTemplate, UpcTemplateDTO upcTemplateDTO) {
    Date date = new Date();
    UpcTemplateTemp upcTemplateTemp = new UpcTemplateTemp();
  ...
  private List<UpcTemplateDataTemp> updateUpcTemplateTempFromMaster(
      UpcTemplateDTO upcTemplateDTO, UpcTemplateTemp upcTemplateTemp) {
    ...
    for (UpcTemplateDataDTO dataDTO : upcTemplateDTO.getUpcTemplateDataDTOList()) {
      UpcSection upcSection =
          upcSectionRepository
              .findById(dataDTO.getUpcSectionID())
              .orElseThrow(
                  () ->
                      new ApiRequestException(
                          "UPC Section with ID " + dataDTO.getUpcSectionID() + DOES_NOT_EXIST));
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Void>> deleteUpcTemplateFromTemp(Integer upcTemplateID)
      throws ApiRequestException {
    log.info("START: Delete UPC Template from Temp with ID: {}", upcTemplateID);
    if (!upcTemplateTempRepository.existsById(upcTemplateID)) {
      log.info(
          "END : UPC Template with ID: {} does not exist. Throwing ApiRequestException.",
          upcTemplateID);
    ...
    StandardResponse<Void> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), upcTemplateID);
