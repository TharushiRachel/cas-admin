...
//                        workflowTemplateDataTemp.setApproveStatus(MasterDataApproveStatus.PENDING);
    ...
    public StandardResponse<Boolean> authorizeWorkflowTemplateTemp(ApproveRejectRQ approveRejectRQ) throws ApiRequestException {
        try {
            ...
            if (approveRejectRQ.getApproveStatus().equals(MasterDataApproveStatus.APPROVED)) {
                ...
                workflowTemplate.setWorkFlowTemplateDataSet(getTempWorkflowTemplateData(workflowTemplateTemp, workflowTemplate));
    ...
    @Override
    public ResponseEntity<StandardResponse<Void>> deleteWorkFlowTempById(Integer approveRejectDataID) throws ApiRequestException {
        try {
            workflowTemplateTempRepository.deleteById(approveRejectDataID);
            StandardResponse<Void> response = new StandardResponse<>
                    (ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), approveRejectDataID);
    ...
    @Override
    public StandardResponse<WorkflowTemplateResponse> getTempWorkflowTemplate(Pageable pageable) throws ApiRequestException {
        try {
    ...
    @Override
    public StandardResponse<Page<WorkflowTemplateDTO>> getWorkflowTemplate(Pageable pageable) throws ApiRequestException {
        try {
            WorkflowTemplateResponse result = new WorkflowTemplateResponse();
            List<WorkflowTemplateDTO> workflowTemplateTempList = new ArrayList<>();
            ...
            Page<WorkflowTemplateDTO> dtoPage = new PageImpl<>(
                    workflowTemplateTempList,
                    pageable,
                    pageDetails.getTotalElements()
            );
            return new StandardResponse<>(ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), dtoPage);
        } catch (Exception e) {
    ...
    public Set<WorkflowTemplateData> getTempWorkflowTemplateData(WorkflowTemplateTemp workflowTemplateTemp, WorkflowTemplate workflowTemplate) {
        try {
