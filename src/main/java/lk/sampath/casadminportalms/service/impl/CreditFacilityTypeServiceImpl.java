...
public class CreditFacilityTypeServiceImpl implements CreditFacilityTypeService {
  ...
  @Override
  public ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> saveCreditFacilityTypeTemp(
      CreditFacilityTypeDTO creditFacilityTypeTempDTO) throws ApiRequestException {
    ...
    try {
      ...
      creditFacilityTypeTemp.setCreditFacilityTypeID(
          creditFacilityTypeTempRepository.getCurrentSequenceValue());
      ...
      CreditFacilityTypeDTO creditFacilityTypeTempDto =
          new CreditFacilityTypeDTO(creditFacilityTypeTemp);
      response =
          new StandardResponse<>(
              ErrorEnums.SUCCESS_CODE.getStatus(),
              ErrorEnums.SUCCESS_CODE.getLabel(),
              creditFacilityTypeTempDto);
    } catch (Exception exception) {

      log.info(
          "END: In Adding Credit facility type threw the exception  with message :{}",
  ...
  private void validateCreditFacilityTypeNameUniquenessAdd(
      CreditFacilityTypeDTO creditFacilityTypeTempDTO) throws ApiRequestException {
    log.info(
        "START: validateCreditFacilityTypeNameUniquenessAdd function in Save Credit Facility type :{}",
        creditFacilityTypeTempDTO);
    if (creditFacilityTypeTempRepository.existsByFacilityTypeName(
        creditFacilityTypeTempDTO.getFacilityTypeName())) {
      log.info(
          "END: validateCreditFacilityTypeNameUniquenessAdd function in Save Credit Facility type threw excpetion for facility name:{}",
          creditFacilityTypeTempDTO.getFacilityTypeName());
    ...
    if (creditFacilityTypeRepository.existsByFacilityTypeName(
        creditFacilityTypeTempDTO.getFacilityTypeName())) {
      log.info(
          "END: validateCreditFacilityTypeNameUniquenessAdd function in Save Credit Facility type threw excpetion for facility name:{}",
          creditFacilityTypeTempDTO.getFacilityTypeName());
  ...
  @Override
  public ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> findCreditFacilityTypeTempByID(
      Integer creditFacilityTypeID) throws ApiRequestException {
    ...
    try {
      CreditFacilityTypeTemp creditFacilityTypeTemp =
          creditFacilityTypeTempRepository
              .findById(creditFacilityTypeID)
              .orElseThrow(
                  () ->
                      new ApiRequestException(
                          CREDIT_FACILITY_TYPE_WITH + creditFacilityTypeID + DOES_NOT_EXISTS));
      ...
      log.info(
          "END: In find by id from temp threw the exception with message :{}",
          exception.getMessage());
      if (exception instanceof ApiRequestException) {
        throw exception;
      } else {
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> updateCreditFacilityTempType(
      Integer creditFacilityTypeID, CreditFacilityTypeDTO creditFacilityTypeTempDTO)
      throws ApiRequestException {

    log.info(
        "START: Update Credit Facility type in Temp :{}, with ID :{}",
        creditFacilityTypeTempDTO,
    ...
    try {
      ...
      validateCreditFacilityTypeNameUniquenessTemp(
          creditFacilityTypeTemp, creditFacilityTypeTempDTO);
      ...
      log.info(
          "END: In update credit facility type in temp threw the exception with message :{}",
          exception.getMessage());
      if (exception instanceof ApiRequestException) {
        throw exception;
      } else {
    ...
    StandardResponse<CreditFacilityTypeDTO> response =
        new StandardResponse<>(
            ...
            new CreditFacilityTypeDTO(creditFacilityTypeTemp));
    log.info(
        "END:  Update Credit Facility type in Temp :{}, with ID :{}",
        creditFacilityTypeTempDTO,
        creditFacilityTypeID);
    return ResponseEntity.ok().body(response);
  }

  private void validateCreditFacilityTypeNameUniquenessTemp(
      CreditFacilityTypeTemp creditFacilityTypeTemp,
      CreditFacilityTypeDTO creditFacilityTypeTempDTO)
      throws ApiRequestException {

    log.info(
        "START: validateCreditFacilityTypeNameUniquenessTemp in update temp :{}, with ID :{}",
        creditFacilityTypeTempDTO,
        creditFacilityTypeTemp.getCreditFacilityTypeID());

    if (!creditFacilityTypeTemp
            .getFacilityTypeName()
            .equals(creditFacilityTypeTempDTO.getFacilityTypeName())
        && creditFacilityTypeTempRepository.existsByFacilityTypeName(
            creditFacilityTypeTempDTO.getFacilityTypeName())) {

      throw new ApiRequestException("Credit Facility Type Name Already in Temporary Records");
    }

    CreditFacilityType creditFacilityTypeMaster =
        creditFacilityTypeRepository.findCreditFacilityTypeByName(
            creditFacilityTypeTempDTO.getFacilityTypeName());
    if (creditFacilityTypeMaster != null
        && !Objects.equals(
            creditFacilityTypeMaster.getCreditFacilityTypeID(),
            creditFacilityTypeTemp.getCreditFacilityTypeID())) {
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> approveRejectCreditFacilityType(
    ...
    try {
      ...
      Optional<CreditFacilityType> optionalCreditFacilityType =
          creditFacilityTypeRepository.findById(creditFacilityTypeTemp.getCreditFacilityTypeID());
      ...
      log.info(
          "SUCCESS: Moved Credit Facility Type Temp with ID :{} to Credit Facility Type with ID :{}",
          creditFacilityTypeTemp.getCreditFacilityTypeID(),
          approveRejectRQ.getApproveRejectDataID());
    } catch (Exception exception) {
      log.info(
          "END: In approve reject credit facility type threw the exception with message :{}",
          exception.getMessage());
      if (exception instanceof ApiRequestException) {
        throw exception;
      } else {
    ...
    log.info(
        "END:  approve reject Credit Facility type in Temp with ID :{}",
        approveRejectRQ.getApproveRejectDataID());
    return response;
  }

  private ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> handleApproval(
      CreditFacilityTypeTemp creditFacilityTypeTemp,
      CreditFacilityType existingCreditFacilityType) {
    log.info(
        "START: Handling approval for Credit Facility Type Temp with ID: {}",
        creditFacilityTypeTemp.getCreditFacilityTypeID());

    CreditFacilityType savedCreditFacilityType;
    try {
      if (existingCreditFacilityType != null
          && existingCreditFacilityType
              .getCreditFacilityTypeID()
              .equals(creditFacilityTypeTemp.getCreditFacilityTypeID())) {

        savedCreditFacilityType =
            updateCreditFacilityTypeToMaster(creditFacilityTypeTemp, existingCreditFacilityType);
      ...
      log.info(
          "Saving audit information for Credit Facility Type Temp with ID: {}",
          creditFacilityTypeTemp.getCreditFacilityTypeID());
      ...
      log.info(
          "Error occurred while handling approval for Credit Facility Type Temp with ID: {}",
          creditFacilityTypeTemp.getCreditFacilityTypeID(),
          e);
    ...
    StandardResponse<CreditFacilityTypeDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(),
            ErrorEnums.SUCCESS_CODE.getLabel(),
            new CreditFacilityTypeDTO(savedCreditFacilityType));
    log.info(
        "END: Returning response for Credit Facility Type with ID: {}",
        savedCreditFacilityType.getCreditFacilityTypeID());

    return ResponseEntity.ok().body(response);
  }

  private ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> handleRejection(
      CreditFacilityTypeTemp creditFacilityTypeTemp) {

    log.info(
        "Handling rejection for UPM Group Temp ID: {}",
        creditFacilityTypeTemp.getFacilityTypeName());
    StandardResponse<CreditFacilityTypeDTO> response;
    try {
      saveCreditFacilityTypeAudit(creditFacilityTypeTemp);
      response =
      ...
      log.info(
          "Error occurred while handling rejection for Credit Facility Type Temp with ID: {}",
          creditFacilityTypeTemp.getCreditFacilityTypeID(),
          e);
    ...
    log.info(
        "END: Rejection handled for Credit Facility Type Temp with ID: {}",
        creditFacilityTypeTemp.getCreditFacilityTypeID());

    return ResponseEntity.ok().body(response);
  }

  private CreditFacilityType updateCreditFacilityTypeToMaster(
      CreditFacilityTypeTemp creditFacilityTypeTemp,
      CreditFacilityType existingCreditFacilityType) {

    Date date = new Date();

    existingCreditFacilityType.setCreditFacilityTypeID(
        creditFacilityTypeTemp.getCreditFacilityTypeID());
  ...
  private CreditFacilityType saveCreditFacilityTypeToMaster(
      CreditFacilityTypeTemp creditFacilityTypeTemp) {
  ...
  private void saveCreditFacilityTypeAudit(CreditFacilityTypeTemp creditFacilityTypeTemp) {
    ...
    log.info(
        "Saved audit record for UPM Group ID: {}",
        creditFacilityTypeTemp.getCreditFacilityTypeID());
  }

  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> updateApprovedCreditFacilityType(
      Integer creditFacilityTypeID, CreditFacilityTypeDTO creditFacilityTypeDTO)
      throws ApiRequestException {
    ...
    try {
      ...
      creditFacilityTypeDb =
          creditFacilityTypeRepository
              .findById(creditFacilityTypeID)
              .orElseThrow(
                  () ->
                      new ApiRequestException(
                          CREDIT_FACILITY_TYPE_WITH
                              + creditFacilityTypeDTO.getCreditFacilityTypeID()
                              + DOES_NOT_EXISTS));
      ...
      creditFacilityTypeTemp.setCreditFacilityTypeID(
          creditFacilityTypeDb.getCreditFacilityTypeID());
      creditFacilityTypeTemp.setLastModifiedDate(date);
      ...
      log.info(
          "END: In update approved credit facility type threw the exception with message :{}",
          exception.getMessage());
      if (exception instanceof ApiRequestException) {
        throw exception;
      } else {
  ...
  private void validateCreditFacilityTypeByIdApprovedTypes(
      CreditFacilityType creditFacilityTypeDb) {
    if (creditFacilityTypeTempRepository.existsByCreditFacilityTypeID(
        creditFacilityTypeDb.getCreditFacilityTypeID())) {
      throw new ApiRequestException("Credit Facility Type Already in Temporary Records");
    }
  }

  private void validateCreditFacilityTypeNameUniqueness(
      CreditFacilityType creditFacilityTypeDb, CreditFacilityTypeDTO creditFacilityTypeDTO)
      throws ApiRequestException {

    if (creditFacilityTypeTempRepository.existsByFacilityTypeName(
        creditFacilityTypeDTO.getFacilityTypeName())) {
      throw new ApiRequestException("Credit Facility Type Name Already in Temporary Records");
    }
    if (!creditFacilityTypeDb
            .getFacilityTypeName()
            .equals(creditFacilityTypeDTO.getFacilityTypeName())
        && creditFacilityTypeRepository.existsByFacilityTypeName(
            creditFacilityTypeDTO.getFacilityTypeName())) {
      throw new ApiRequestException("Credit Facility Type Name Already in Master Records");
    }
  }
  ...
  @Transactional(readOnly = true)
  @Override
  public ResponseEntity<StandardResponse<List<CreditFacilityTypeDTO>>> searchCreditFacilityTypes(
    ...
    Page<CreditFacilityType> creditFacilityTypePage =
        creditFacilityTypeRepository.findAll(pageable);
    StandardResponse<List<CreditFacilityTypeDTO>> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(),
            ErrorEnums.SUCCESS_CODE.getLabel(),
            creditFacilityTypePage);
  ...
  @Transactional(readOnly = true)
  @Override
  public ResponseEntity<StandardResponse<CreditFacilityTypeDTO>> findCreditFacilityTypeByID(
      Integer creditFacilityTypeID) throws ApiRequestException {
    ...
    try {
      ...
      log.info(
          "END: Credit Facility Types in Master threw the exception with message :{}",
          exception.getMessage());
      if (exception instanceof ApiRequestException) {
        throw exception;
      } else {
  ...
  @Transactional(readOnly = true)
  @Override
  public ResponseEntity<StandardResponse<List<CreditFacilityTypeDTO>>>
      findAllCreditFacilityTypeTempList(Pageable pageable) throws ApiRequestException {
    log.info("START : GET Credit Facility Types List in Temp ");
    Page<CreditFacilityTypeTemp> creditFacilityTypeTempPage =
        creditFacilityTypeTempRepository.findAll(pageable);
    StandardResponse<List<CreditFacilityTypeDTO>> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(),
            ErrorEnums.SUCCESS_CODE.getLabel(),
            creditFacilityTypeTempPage);
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Integer>> deleteCreditFacilityTypeTemp(
      CreditFacilityTypeDTO creditFacilityTypeTempDTO) throws ApiRequestException {
    ...
    try {
      creditFacilityTypeTemp =
          creditFacilityTypeTempRepository
              .findById(creditFacilityTypeTempDTO.getCreditFacilityTypeID())
              .orElseThrow(
                  () ->
                      new ApiRequestException(
                          CREDIT_FACILITY_TYPE_WITH
                              + creditFacilityTypeTempDTO.getCreditFacilityTypeID()
                              + DOES_NOT_EXISTS));
      ...
      log.info(
          "END: Credit Facility Types Delete threw the exception with message :{}",
          exception.getMessage());
      if (exception instanceof ApiRequestException) {
        throw exception;
      } else {
    ...
    StandardResponse<Integer> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(),
            ErrorEnums.SUCCESS_CODE.getLabel(),
            creditFacilityTypeTemp.getCreditFacilityTypeID());
    log.info(
        "END : Delete Credit Facility Type with ID {} ",
        creditFacilityTypeTempDTO.getCreditFacilityTypeID());
    return ResponseEntity.ok().body(response);
  }
}
