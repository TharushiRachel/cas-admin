...
public class UserDaServiceImpl implements UserDaService {
  ...
  @Autowired private UserDaRepository userDaRepository;

  @Autowired private UserDaTempRepository userDaTempRepository;

  @Autowired private UserDaAudRepository userDaAudRepository;

  @Override
  public ResponseEntity<StandardResponse<List<UserDaDTO>>> findAllUserDaTempList(Pageable pageable)
  ...
  @Override
  public ResponseEntity<StandardResponse<UserDaDTO>> findUserDaTempByID(Integer userDaID)
      throws ApiRequestException {
    UserDaTemp userDaTemp =
        userDaTempRepository
            .findById(userDaID)
            .orElseThrow(
                () -> new ApiRequestException(USER_DA_TEMP_WITH + userDaID + DOES_NOT_EXISTS));
    StandardResponse<UserDaDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), userDaTemp);
    return ResponseEntity.ok().body(response);
  }
  ...
  public ResponseEntity<StandardResponse<List<UserDaDTO>>> findAllApprovedUserDa(
    ...
    StandardResponse<List<UserDaDTO>> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), userDaList);
    return ResponseEntity.ok().body(response);
  }

  @Override
  @Transactional(readOnly = true)
  public ResponseEntity<StandardResponse<UserDaDTO>> findApprovedUserDaById(Integer userDaID)
      throws ApiRequestException {
    UserDa userDa =
        userDaRepository
            .findById(userDaID)
    ...
    StandardResponse<UserDaDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), userDa);
    return ResponseEntity.ok().body(response);
  }

  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<UserDaDTO>> saveUserDaTemp(UserDaDTO userDaDTO)
      throws ApiRequestException {

    log.info("START: save UserDA :{}", userDaDTO);

    if (userDaDTO == null
        || userDaDTO.getUserName() == null
        || userDaDTO.getUserName().trim().isEmpty()) {
      throw new ApiRequestException("User cannot be empty or null.");
    }

    if (userDaDTO.getCleanAmount() != null
        && userDaDTO.getCleanAmount().compareTo(BigDecimal.ZERO) < 0) {
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<UserDaDTO>> approveRejectUserDa(
  ...
  private ResponseEntity<StandardResponse<UserDaDTO>> handleApproval(
      UserDaTemp temp, UserDa existingUserDa) {
    UserDa savedUserDa;
    ...
    StandardResponse<UserDaDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), savedUserDa);
    return ResponseEntity.ok().body(response);
  }
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<UserDaDTO>> updateUserDaTemp(
      Integer userDaID, UserDaDTO userDaDTO) throws ApiRequestException {
    ...
    BooleanBuilder booleanBuilder = new BooleanBuilder();
    booleanBuilder.and(QUserDaTemp.userDaTemp.userName.eq(userDaDTO.getUserName()));
    List<UserDaTemp> userDaTempList =
        (List<UserDaTemp>) userDaTempRepository.findAll(booleanBuilder);
    ...
    boolean existingTempList =
        userDaTempList.stream()
            .anyMatch(temp -> temp.getUserName().equals(userDaDTO.getUserName()));
    if (existingTempList && !userDaDb.getUserName().equals(userDaDTO.getUserName())) {
      throw new ApiRequestException(
          USER_DA_TEMP_WITH + userDaDTO.getUserName() + " Already Exists");
    }
    ...
    StandardResponse<UserDaDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), userDaDb);
    return ResponseEntity.ok().body(response);
  }

  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<UserDaDTO>> updateApprovedUserDa(
      Integer userDaID, UserDaDTO userDaDTO) throws ApiRequestException {
  ...
  private void validateUserDaNameUniqueness(String userDaName, Integer userDaID)
      throws ApiRequestException {
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Void>> deleteUserDaFromTemp(Integer userDaID)
      throws ApiRequestException {
    userDaTempRepository.deleteById(userDaID);
    StandardResponse<Void> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), userDaID);
    return ResponseEntity.ok().body(response);
  }
}
