...
public class UpcSectionServiceImpl implements UpcSectionService {
  ...
  public UpcSectionServiceImpl(
      UpcSectionRepository upcSectionRepository,
      UpcSectionTempRepository upcSectionTempRepository,
      UpcSectionAudRepository upcSectionAudRepository) {
  ...
  public ResponseEntity<StandardResponse<List<UpcSectionDTO>>> findAllUpcSectionTempList(
      Pageable pageable) throws ApiRequestException {
    log.info(
        "START: UpcSectionServiceImpl | findAllUpcSectionTempList with pageable: {}", pageable);
  ...
  @Override
  @Transactional(readOnly = true)
  public ResponseEntity<StandardResponse<UpcSectionDTO>> findUpcSectionTempByID(
      Integer upcSectionID) throws ApiRequestException {
    log.info(
        "START: UpcSectionServiceImpl | findUpcSectionTempByID with upcSectionID: {}",
        upcSectionID);
    UpcSectionTemp upcSectionTemp =
        upcSectionTempRepository
            .findById(upcSectionID)
            .orElseThrow(
                () ->
                    new ApiRequestException(
                        UPC_SECTION_TEMP_WITH + upcSectionID + DOES_NOT_EXISTS));
  ...
  @Override
  @Transactional(readOnly = true)
  public ResponseEntity<StandardResponse<UpcSectionDTO>> findApprovedUpcSectionByID(
      Integer upcSectionID) throws ApiRequestException {
    log.info("START: findApprovedUpcSectionByID with upcSectionID: {}", upcSectionID);
    UpcSection upcSection =
        upcSectionRepository
            .findById(upcSectionID)
            .orElseThrow(
                () -> {
    ...
    StandardResponse<UpcSectionDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), upcSection);
  ...
  public ResponseEntity<StandardResponse<UpcSectionDTO>> saveUpcSectionTemp(
      UpcSectionDTO upcSectionDTO) throws ApiRequestException {
    log.info(
        "START: UpcSectionServiceImpl | saveUpcSectionTemp with upcSectionDTO: {}", upcSectionDTO);

    if (upcSectionDTO == null
        || upcSectionDTO.getUpcSectionName() == null
        || upcSectionDTO.getUpcSectionName().trim().isEmpty()) {
    ...
    booleanBuilder.and(
        QUpcSectionTemp.upcSectionTemp.upcSectionName.eq(upcSectionDTO.getUpcSectionName()));
    List<UpcSectionTemp> upcSectionTemps =
        (List<UpcSectionTemp>) upcSectionTempRepository.findAll(booleanBuilder);
  ...
  public ResponseEntity<StandardResponse<UpcSectionDTO>> approveRejectUpcSection(
    ...
    log.info(
        "START: UpcSectionServiceImpl | approveRejectUpcSection with ApproveRejectRQ: {}",
        approveRejectRQ);
    ...
    UpcSectionTemp upcSectionTemp =
        upcSectionTempRepository
            ...
            .orElseThrow(
                () ->
                    new ApiRequestException(
                        UPC_SECTION_TEMP_WITH
                            + approveRejectRQ.getApproveRejectDataID()
                            + DOES_NOT_EXISTS));

    Optional<UpcSection> optionalUpcSection =
        upcSectionRepository.findById(upcSectionTemp.getUpcSectionID());
  ...
  private ResponseEntity<StandardResponse<UpcSectionDTO>> handleApproval(
      UpcSectionTemp temp, UpcSection existingUpcSection) {
    log.info(
        "UpcSectionServiceImpl | Handling approval for UPC Section Temp ID: {} ",
        temp.getUpcSectionID());
    UpcSection savedUpcSection;

    if (existingUpcSection != null
        && existingUpcSection.getUpcSectionID().equals(temp.getUpcSectionID())) {
    ...
    log.info(
        "Handling Approval for supporting doc temp ID: {} ", savedUpcSection.getUpcSectionID());
    StandardResponse<UpcSectionDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(),
            ErrorEnums.SUCCESS_CODE.getLabel(),
  ...
  private ResponseEntity<StandardResponse<UpcSectionDTO>> handleRejection(UpcSectionTemp temp) {
    ...
    StandardResponse<UpcSectionDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), temp);
    return ResponseEntity.ok().body(response);
  }

  private UpcSection updateUpcSectionToMaster(
      UpcSectionTemp upcSectionTemp, UpcSection upcSection) {

    log.info(
        "UpcSectionServiceImpl | Updating existing UPC Section ID: {} to master with data from temp ID: {}",
        upcSection.getUpcSectionID(),
        upcSectionTemp.getUpcSectionID());
    Date date = new Date();
    ...
    log.info(
        "UpcSectionServiceImpl | Updated existing UPC Section ID: {} to master with data from temp ID: {}",
        upcSection.getUpcSectionID(),
        upcSectionTemp.getUpcSectionID());
  ...
  private UpcSection mapUpcSection(UpcSectionTemp upcSectionTemp, UpcSection existingUpcSection) {
    log.info(
        "UpcSectionServiceImpl | Mapping UPC Section Temp ID: {} to master. Existing UPC Section: {}",
        upcSectionTemp.getUpcSectionID(),
        existingUpcSection);
    ...
    log.info(
        "UpcSectionServiceImpl | Mapped UPC Section Temp ID: {} to master. Resulting UPC Section ID: {}",
        upcSectionTemp.getUpcSectionID(),
        upcSection.getUpcSectionID());
    return upcSection;
  }
  ...
  private void saveUpcSectionAudit(UpcSectionTemp temp) {
    ...
    log.info(
        "UpcSectionServiceImpl | saveUpcSectionAudit | saved audit record for UPC Section ID: {}",
        temp.getUpcSectionID());
  }
  ...
  public ResponseEntity<StandardResponse<UpcSectionDTO>> updateUpcSectionTemp(
    ...
    log.info(
        "START: UpcSectionServiceImpl | updateUpcSectionTemp | Update Upc Section: {}",
        upcSectionDTO);
    Date date = new Date();

    UpcSectionTemp upcSectionDb =
        upcSectionTempRepository
            .findById(upcSectionID)
            .orElseThrow(
                () -> {
    ...
    booleanBuilder.and(
        QUpcSectionTemp.upcSectionTemp.upcSectionName.eq(upcSectionDTO.getUpcSectionName()));
    List<UpcSectionTemp> upcSectionTempList =
        (List<UpcSectionTemp>) upcSectionTempRepository.findAll(booleanBuilder);
    ...
    } else if (upcSectionDTO.getUpcSectionName() == null
        || upcSectionDTO.getUpcSectionName().trim().isEmpty()) {
      throw new ApiRequestException(EMPTY_NULL);
    }

    // Use a stream to check if any element in the list matches the provided string
    boolean existsInTempList =
        upcSectionTempList.stream()
    ...
    if (existsInTempList
      ...
      throw new ApiRequestException(
          UPC_SECTION_TEMP_WITH + upcSectionDTO.getUpcSectionName() + " Already Exists");
    }
    ...
    StandardResponse<UpcSectionDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), upcSectionDb);

    log.info(
        "END: UpcSectionServiceImpl | updateUpcSectionTemp | Update Upc Section response: {}",
        response);
    return ResponseEntity.ok().body(response);
  }
  ...
  public ResponseEntity<StandardResponse<UpcSectionDTO>> updateApprovedUpcSection(
      Integer upcSectionID, UpcSectionDTO upcSectionDTO) throws ApiRequestException {
    log.info(
        "START: UpcSectionServiceImpl | updateApprovedUpcSection | Update UPC Section :{}",
        upcSectionDTO);

    UpcSection upcSectionDb =
        upcSectionRepository
            .findById(upcSectionID)
            .orElseThrow(
                () -> {
    ...
    log.info(
        "END: UpcSectionServiceImpl | updateApprovedUpcSection | Update UPC Section response: {}",
        response);
    return ResponseEntity.ok().body(response);
  }

  private void validateUpcSectionNameUniqueness(String upcSectionName, Integer upcSectionID)
      throws ApiRequestException {

    log.info(
        "UpcSectionServiceImpl | validateUpcSectionNameUniqueness | Validating uniqueness of UPC Section Name: {} with ID: {}",
        upcSectionName,
        upcSectionID);
    ...
    if (existsInTemp || existsInMaster) {
      throw new ApiRequestException(
          "UPC Section Name '" + upcSectionName + "' already exists in the system.");
    }

    log.info(
        "UpcSectionServiceImpl | validateUpcSectionNameUniqueness | UPC Section Name: {} is unique.",
        upcSectionName);
  }
  ...
  @Override
  @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = ApiRequestException.class)
  public ResponseEntity<StandardResponse<Void>> deleteUpcSectionFormTemp(Integer upcSectionID)
      throws ApiRequestException {
    log.info(
        "START: UpcSectionServiceImpl | deleteUpcSectionFormTemp | Deleting UPC Section Temp with ID: {}",
        upcSectionID);
    upcSectionTempRepository.deleteById(upcSectionID);
    StandardResponse<Void> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), upcSectionID);
    log.info(
        "END: UpcSectionServiceImpl | deleteUpcSectionFormTemp | Deleted UPC Section Temp with ID: {}",
        upcSectionID);
    return ResponseEntity.ok().body(response);
  }
  ...
  public ResponseEntity<StandardResponse<UpcSectionDTO>> approvedActiveList()
      throws ApiRequestException {
    log.info(
        "START: UpcSectionServiceImpl | approvedActiveList | Fetching all approved and active UPC Sections");
    List<UpcSection> activeApprovedSections =
        upcSectionRepository.findByStatusAndApproveStatus(
            Status.ACT, MasterDataApproveStatus.APPROVED);

    List<UpcSectionDTO> resultList =
        activeApprovedSections.stream()
            .map(
                upcSection -> {
    ...
    StandardResponse<UpcSectionDTO> response =
        new StandardResponse<>(
            ErrorEnums.SUCCESS_CODE.getStatus(), ErrorEnums.SUCCESS_CODE.getLabel(), resultList);
    return ResponseEntity.ok().body(response);
  }
}
