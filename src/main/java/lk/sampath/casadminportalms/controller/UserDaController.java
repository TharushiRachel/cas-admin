...
public class UserDaController {
  ...
  @Autowired private UserDaService userDaService;
  ...
  public ResponseEntity<StandardResponse<List<UserDaDTO>>> viewAllUserDaTemp(
    ...
    ResponseEntity<StandardResponse<List<UserDaDTO>>> userDaTempList =
  ...
  public ResponseEntity<StandardResponse<UserDaDTO>> viewUserDaTempById(
      @PathVariable Integer userDaID) throws ApiRequestException {
    ResponseEntity<StandardResponse<UserDaDTO>> userDaTempDTO =
        userDaService.findUserDaTempByID(userDaID);
  ...
  public ResponseEntity<StandardResponse<List<UserDaDTO>>> getPagedUserDaData(
      ...
      throws ApiRequestException {
    Pageable pageable = PaginationUtil.createPageable(headerPage, headerSize, page, size);
    ResponseEntity<StandardResponse<List<UserDaDTO>>> userDaList =
        userDaService.findAllApprovedUserDa(pageable);
    return ResponseEntity.ok().body(userDaList.getBody());
  }

  @GetMapping("/{userDaID}")
  public ResponseEntity<StandardResponse<UserDaDTO>> viewUserDaById(@PathVariable Integer userDaID)
      throws ApiRequestException {
    ResponseEntity<StandardResponse<UserDaDTO>> userDa =
        userDaService.findApprovedUserDaById(userDaID);
    return ResponseEntity.ok().body(userDa.getBody());
  }

  @PostMapping("/saveUserDa")
  public ResponseEntity<StandardResponse<UserDaDTO>> saveUserDa(
      @Validated @RequestBody UserDaDTO request) throws ApiRequestException {
  ...
  public ResponseEntity<StandardResponse<UserDaDTO>> updateUserDaTemp(
      @PathVariable Integer userDaID, @Validated @RequestBody UserDaDTO request)
      throws ApiRequestException {
    ResponseEntity<StandardResponse<UserDaDTO>> userDa =
        userDaService.updateUserDaTemp(userDaID, request);
    return ResponseEntity.ok().body(userDa.getBody());
  }

  @PostMapping("/userDa/{userDaID}")
  public ResponseEntity<StandardResponse<UserDaDTO>> updateApprovedUserDa(
      @PathVariable Integer userDaID, @Validated @RequestBody UserDaDTO userDaDTO)
      throws ApiRequestException {
    ResponseEntity<StandardResponse<UserDaDTO>> userDa =
        userDaService.updateApprovedUserDa(userDaID, userDaDTO);
    return ResponseEntity.ok().body(userDa.getBody());
  }

  @PostMapping("/userDaTemp/deleteUserDaTemp")
  public ResponseEntity<StandardResponse<Void>> deleteUserDaTemp(
      @Validated @RequestBody UserDaDTO request) throws ApiRequestException {
    ResponseEntity<StandardResponse<Void>> userDa =
        userDaService.deleteUserDaFromTemp(request.getUserDaID());
    return ResponseEntity.ok().body(userDa.getBody());
  }
}
