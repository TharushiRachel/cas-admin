...
class SupportingDocControllerTest {
    ...
    @InjectMocks
    private SupportingDocController supportingDocController;
    ...
    void testViewAllSupportingDocTemp() throws Exception {
        ...
        StandardResponse<List<SupportingDocDTO>> response = new StandardResponse<>(true, "Success", supportingDocList);
        when(supportingDocService.findAllSupportingDocTempList()).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(viewSupportingDocTempList)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.response[0].documentName").value("Unit Testing"))
    ...
    void testViewAllSupportingDocTemp_EmptyList() throws Exception{
        ...
        StandardResponse<List<SupportingDocDTO>> response = new StandardResponse<>(true,"Success", supportingDocDTOS);
        when(supportingDocService.findAllSupportingDocTempList()).thenReturn(ResponseEntity.ok(response));
    ...
    void testViewSupportingDocTempById_Success() throws Exception{
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(true, "Success", pendingSupportingDoc);
        when(supportingDocService.findSupportingDocTempById(1)).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(supportingDocTempViewById,1)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void testViewAllSupportingDoc_Success() throws Exception{
        ...
        StandardResponse<List<SupportingDocDTO>> response = new StandardResponse<>(true, "Success", supportingDocDTOList);

        when(supportingDocService.searchSupportingDocGroups()).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(viewSupportingDocList)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void testViewSupportingDocId_Success() throws Exception{
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(true, "Success", approvedSupportingDoc);
        when(supportingDocService.findSupportingDocById(1)).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(viewSupportingDocById, 1)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.response.documentName").value("Unit Testing"))
                .andExpect(jsonPath("$.response.description").value("Unit Testing Description"))
    ...
    void testSaveSupporting_Success() throws Exception{
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(true, "Saved Successfully", pendingSupportingDoc);

        when(supportingDocService.saveSupportingDocTemp(any(SupportingDocDTO.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(saveSupportingDoc)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void testSupportingDocSave_Failure() throws Exception{
        ...
        doThrow(new ApiRequestException("Validation failed")).when(supportingDocService).saveSupportingDocTemp(any(SupportingDocDTO.class));
    ...
    void testApproveRejectSupportingDoc_Success() throws Exception{
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(true, "APPROVED", approvedSupportingDoc);
        when(supportingDocService.approveRejectSupportingDoc(any(ApproveRejectRQ.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(approveRejectSupportingDoc)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void testApproveRejectSupportingDoc_Failure() throws Exception{
        doThrow(new ApiRequestException("Approval failed")).when(supportingDocService).approveRejectSupportingDoc(any(ApproveRejectRQ.class));
    ...
    void testUpdateSupportingDocTemp_Success() throws Exception{
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(true, "Updated", pendingSupportingDoc);

        when(supportingDocService.updateSupportingDocTemp(eq(1), any(SupportingDocDTO.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(updateSupportingDocTemp, 1)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(pendingSupportingDoc)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message" ).value("Updated"));

        verify(supportingDocService, times(1)).updateSupportingDocTemp(eq(1), any(SupportingDocDTO.class));
    }

    @Test
    void testUpdateSupportingDocTemp_Failure() throws Exception{
        doThrow(new ApiRequestException("Update failed")).when(supportingDocService).updateSupportingDocTemp(eq(1), any(SupportingDocDTO.class));
    ...
    void testUpdateApprovedSupportingDoc_Success() throws Exception{
        StandardResponse<SupportingDocDTO> response = new StandardResponse<>(true, "Updated", approvedSupportingDoc);

        when(supportingDocService.updateApprovedSupportingDoc(eq(1), any(SupportingDocDTO.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(updateApprovedSupportingDoc, 1)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(approvedSupportingDoc)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Updated"));

        verify(supportingDocService, times(1)).updateApprovedSupportingDoc(eq(1), any(SupportingDocDTO.class));
    }

    @Test
    void testUpdateApprovedSupportingDoc_Failure() throws Exception{
        doThrow(new ApiRequestException("Update Failed")).when(supportingDocService).updateApprovedSupportingDoc(eq(1), any(SupportingDocDTO.class));
    ...
    void testDeleteSupportingDocTemp_Success() throws Exception{
        ...
        mockMvc.perform(post(deleteSupportingDocTemp)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(pendingSupportingDoc)))
                .andExpect(status().isOk())
    ...
    void testDeleteSupportingDocTemp_Failure() throws Exception{
        doThrow(new ApiRequestException("Deletion Failed")).when(supportingDocService).deleteSupportingDocTemp(1);
