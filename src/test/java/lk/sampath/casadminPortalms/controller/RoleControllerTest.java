...
class RoleControllerTest {
    ...
    void testFindAllPrivilegeCategories() throws Exception {
        ...
        StandardResponse<List<PrivilegeCategory>> response = new StandardResponse<>(true, "Success", mockPrivilegeCategories);
        when(roleService.findAllPrivilegeCategories()).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get("/api/role/getSystemPrivileges"))
    ...
    void testViewTempRoleById() throws Exception {
        ...
        mockMvc.perform(get("/api/role/roleTemp/{roleID}", mockRoleID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void testViewTempRoleList() throws Exception {
        ...
        StandardResponse<List<RoleDTO>> response = new StandardResponse<>(true, "Success", mockRoleList);
        when(roleService.findAllRolesTempList()).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get("/api/role/roleTempList"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("Success"));
    }
    ...
    void testViewRoleById() throws Exception {
        ...
        mockMvc.perform(get("/api/role/{roleID}", mockRoleID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("Success"));
    }
    ...
    void testViewRoleList() throws Exception {
        ...
        mockMvc.perform(get("/api/role/searchRoles"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("Success"));
    }
    ...
    void testSaveRole_Success() throws Exception {
        ...
        StandardResponse<Object> response = new StandardResponse<>(true, "Role saved successfully", savedRole);
    ...
    void testSaveRole_InvalidInput() throws Exception {
        ...
        when(roleService.saveRoleTemp(any(RoleDTO.class))).thenThrow(new ApiRequestException("Role name cannot be null"));
    ...
    void testUpdateRole_Success() throws Exception {
        ...
        StandardResponse<Object> response = new StandardResponse<>(true, "Role updated successfully", updatedRole);
        when(roleService.updateRoleTemp(eq(roleID), any(RoleDTO.class))).thenReturn(ResponseEntity.ok(response));

        // Act & Assert
        mockMvc.perform(post("/api/role/updateTempRole/{roleID}", roleID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void testApproveRejectRole_Success() throws Exception {
        ...
        StandardResponse<Object> response = new StandardResponse<>(true, "Role approved successfully", null);
        when(roleService.approveRejectRole(any(ApproveRejectRQ.class))).thenReturn(ResponseEntity.ok(response));

        // Act & Assert
        mockMvc.perform(post("/api/role/approvedRejectRole")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void testUpdateApprovedRole_Success() throws Exception {
        ...
        StandardResponse<Object> response = new StandardResponse<>(true, "Role updated successfully", null);
        when(roleService.updateApprovedRole(eq(roleID), any(RoleDTO.class))).thenReturn(ResponseEntity.ok(response));

        // Act & Assert
        mockMvc.perform(post("/api/role/updateApprovedRole/{roleID}", roleID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void testDeleteRoleTempById_Success() throws Exception {
        ...
        StandardResponse<Void> response = new StandardResponse<>(true, "Role deleted successfully", null);
        when(roleService.deleteRoleTempById(roleDTO.getRoleID())).thenReturn(ResponseEntity.ok(response));

        // Act & Assert
        mockMvc.perform(post("/api/role/deleteRoleTemp")
                        .contentType(MediaType.APPLICATION_JSON)
