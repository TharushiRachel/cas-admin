...
class CreditFacilityTypeControllerTest {
    ...
    @InjectMocks
    private CreditFacilityTypeController creditFacilityTypeController;
    ...
    void testSaveCreditFacilityTypeSuccess() throws Exception {
        ...
        StandardResponse<CreditFacilityTypeDTO> response = new StandardResponse<>(true, "Saved successfully", tempCreditFacilityType);

        when(creditFacilityTypeService.saveCreditFacilityTypeTemp(any(CreditFacilityTypeDTO.class)))
                .thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(saveCreditFacilityType)
    ...
    void testSaveCreditFacilityTypeBadRequest() throws Exception {
        ...
        verify(creditFacilityTypeService, times(1)).saveCreditFacilityTypeTemp(any(CreditFacilityTypeDTO.class));
    }
    ...
    void testGetCreditFacilityTypeTempById_Success() throws Exception {
        StandardResponse<CreditFacilityTypeDTO> response = new StandardResponse<>(true,"Success", tempCreditFacilityType);
        when(creditFacilityTypeService.findCreditFacilityTypeTempByID(1)).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(getCreditFacilityTypeTempById, 1)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void testUpdateCreditFacilityTypeTemp_Success() throws Exception {
        ...
        when(creditFacilityTypeService.updateCreditFacilityTempType(eq(1), any(CreditFacilityTypeDTO.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(updateCreditFacilityTypeTemp, 1)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(creditFacilityTypeService, times(1)).updateCreditFacilityTempType(eq(1), any(CreditFacilityTypeDTO.class));
    }

    @Test
    void testUpdateCreditFacilityTypeTemp_Failure() throws Exception {
        doThrow(new ApiRequestException("Update failed")).when(creditFacilityTypeService).updateCreditFacilityTempType(eq(1), any(CreditFacilityTypeDTO.class));
    ...
    void testApproveRejectCreditFacilityType_Success() throws Exception {
        StandardResponse<CreditFacilityTypeDTO> response = new StandardResponse<>(true,"Success", approvedCreditFacilityType);

        when(creditFacilityTypeService.approveRejectCreditFacilityType(any(ApproveRejectRQ.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(approveRejectCreditFacilityType)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(creditFacilityTypeService, times(1)).approveRejectCreditFacilityType(any(ApproveRejectRQ.class));
    }

    @Test
    void testApproveRejectCreditFacilityType_Failure() throws Exception {
        doThrow(new ApiRequestException("Approval failed")).when(creditFacilityTypeService).approveRejectCreditFacilityType(any(ApproveRejectRQ.class));
    ...
    void testGetCreditFacilityTypeMasterList_Success() throws Exception {
        List<CreditFacilityTypeDTO> approvedCreditFacilityTypeList = List.of(approvedCreditFacilityType);
        StandardResponse<List<CreditFacilityTypeDTO>> response = new StandardResponse<>(true,"Success", approvedCreditFacilityTypeList);

        when(creditFacilityTypeService.searchCreditFacilityTypes()).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(getCreditFacilityTypeMasterList)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.response[0].facilityTypeName").value("Unit Testing"))
    ...
    void testGetCreditFacilityTypeTempList_Success() throws Exception {
        ...
        StandardResponse<List<CreditFacilityTypeDTO>> response = new StandardResponse<>(true,"Success", tempCreditFacilityTypeList);

        when(creditFacilityTypeService.findAllCreditFacilityTypeTempList()).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(getCreditFacilityTypeTempList)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.response[0].facilityTypeName").value("Unit Testing"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.response[0].description").value("Unit Testing Description"))
                .andExpect(jsonPath("$.response[0].approveStatus").value(MasterDataApproveStatus.PENDING.name()));
    ...
    void testGetCreditFacilityTypeById_Success() throws Exception {
        StandardResponse<CreditFacilityTypeDTO> response = new StandardResponse<>(true,"Success", approvedCreditFacilityType);
        when(creditFacilityTypeService.findCreditFacilityTypeByID(1)).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get(getCreditFacilityTypeId, 1)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.response.facilityTypeName").value("Unit Testing"))
    ...
    void testDeleteCreditFacilityTypeTemp_Success() throws Exception {
        ...
        when(creditFacilityTypeService.deleteCreditFacilityTypeTemp(tempCreditFacilityType)).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(deleteCreditFacilityTypeTemp)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(tempCreditFacilityType)))
                .andExpect(status().isOk())
        ...
        verify(creditFacilityTypeService, times(1)).deleteCreditFacilityTypeTemp(tempCreditFacilityType);
    }

    @Test
    void testDeleteCreditFacilityTypeTemp_Failure() throws Exception {
        doThrow(new ApiRequestException("Deletion failed")).when(creditFacilityTypeService).deleteCreditFacilityTypeTemp(tempCreditFacilityType);
    ...
    void testUpdateCreditFacilityTypeMaster_Success() throws Exception {
        ...
        when(creditFacilityTypeService.updateApprovedCreditFacilityType(eq(1), any(CreditFacilityTypeDTO.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post(updateCreditFacilityTypeMaster, 1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(tempCreditFacilityType)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Success"))
        ...
        verify(creditFacilityTypeService, times(1)).updateApprovedCreditFacilityType(eq(1), any(CreditFacilityTypeDTO.class));
    }

    @Test
    void testUpdateCreditFacilityTypeMaster_Failure() throws Exception {
        doThrow(new ApiRequestException("Update failed")).when(creditFacilityTypeService).updateApprovedCreditFacilityType(eq(1), any(CreditFacilityTypeDTO.class));
