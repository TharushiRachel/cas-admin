...
class UpcSectionControllerTest {
    ...
    void testViewAllUpcSectionTemp() throws Exception {
        ...
        StandardResponse<List<UpcSectionDTO>> response = new StandardResponse<>(true, "Success", mockResponse);

        when(upcSectionService.findAllUpcSectionTempList())
                .thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get("/api/upcSection/upcSectionTemp"))
    ...
    void testViewUpcSectionTempById() throws Exception {
        ...
        when(upcSectionService.findUpcSectionTempByID(mockId))
                .thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get("/api/upcSection/upcSectionTemp/" + mockId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.response.upcSectionID").value(mockId)); // Validate the specific field
    ...
    void testGetPagedUpcSectionData() throws Exception {
        ...
        when(upcSectionService.findAllApprovedUpcSection())
                .thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(get("/api/upcSection/upcSectionList"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.response").isArray());
    ...
    void testSaveUpcSection() throws Exception {
        ...
        StandardResponse<UpcSectionDTO> response = new StandardResponse<>(true, "Success", mockResponse);

        when(upcSectionService.saveUpcSectionTemp(any()))
                .thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(MockMvcRequestBuilders.post("/api/upcSection")
    ...
    void testApproveRejectUpcSection() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/upcSection/approveRejectUpcSection")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(mockRequest)))
                .andExpect(status().isOk());
    ...
    void testUpdateUpcSectionTemp() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/upcSection/updateUpcSectionTemp/" + mockId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(mockRequest)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.response.upcSectionID").value(mockId));

        Mockito.verify(upcSectionService, times(1)).updateUpcSectionTemp(Mockito.eq(mockId), Mockito.any());
    }
    ...
    void testDeleteUpcSectionTemp() throws Exception {
        ...
        Mockito.when(upcSectionService.deleteUpcSectionFormTemp(mockRequest.getUpcSectionID()))
                .thenReturn(ResponseEntity.ok(new StandardResponse<>(true, "Success", mockRequest.getUpcSectionID())));

        mockMvc.perform(MockMvcRequestBuilders.post("/api/upcSection/upcSectionTemp/deleteUpcSectionTemp")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(mockRequest)))
                .andExpect(status().isOk());

        Mockito.verify(upcSectionService, times(1)).deleteUpcSectionFormTemp(mockRequest.getUpcSectionID());
    }
    ...
    void testViewUpcSectionById_Success() throws Exception {
        ...
        mockMvc.perform(get("/api/upcSection/"+ mockId)
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    void testUpdateApprovedUpcSection_Success() throws Exception {
        ...
        when(upcSectionService.updateApprovedUpcSection(eq(1), any(UpcSectionDTO.class))).thenReturn(ResponseEntity.ok(response));

        mockMvc.perform(post("/api/upcSection/updateUpcSectionMaster/"+ 1)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void testUpdateApprovedUpcSection_Failure() throws Exception {
        doThrow(new ApiRequestException("Update failed")).when(upcSectionService).updateApprovedUpcSection(eq(1), any(UpcSectionDTO.class));
