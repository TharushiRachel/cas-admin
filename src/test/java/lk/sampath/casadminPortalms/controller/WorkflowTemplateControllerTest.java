...
class WorkflowTemplateControllerTest {
    ...
    void testGetAllApprovedUPMGroups() throws Exception {
        ...
        StandardResponse<List<UpmGroupDTO>> response = new StandardResponse<>(true, "Success", mockList);
        when(workflowTemplateService.getAllApprovedUPMGroups()).thenReturn(response);

        mockMvc.perform(get("/api/workflowTemplate/getAllApprovedUPMGroups"))
    ...
    void testSaveOrUpdateTempWorkflowTemplate() throws Exception {
        StandardResponse<String> response = new StandardResponse<>(true, "Success", workflowTemplateDTO);
        when(workflowTemplateService.saveOrUpdateTempWorkflowTemplate(workflowTemplateDTO)).thenReturn(response);

        mockMvc.perform(post("/api/workflowTemplate/saveOrUpdateTempWorkflowTemplate")
    ...
    void testAuthorizeWorkflowTemplateTemp() throws Exception {
        ...
        when(workflowTemplateService.authorizeWorkflowTemplateTemp(approveRejectRQ)).thenReturn(response);

        mockMvc.perform(post("/api/workflowTemplate/authorizeWorkFlowTemp")
                        .contentType(MediaType.APPLICATION_JSON)
