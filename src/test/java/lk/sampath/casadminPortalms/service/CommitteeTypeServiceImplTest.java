...
class CommitteeTypeServiceImplTest {
  ...
  @Mock CommitteeTypeDTO committeeTypeDTO;

  @Mock CommitteeType committeeType;
  ...
  void testGetCommitteeTypes() {
    //        List<CommitteeType> mockCommiteeTypeList = List.of(new CommitteeType(), new
    // CommitteeType());
    ...
    //        assertEquals(new StandardResponse<>(ErrorEnums.SUCCESS_CODE.getStatus(),
  ...
  void testUpdateCommitteeType() {
    ...
    StandardResponse<List<CommitteeTypeDTO>> response =
        committeeTypeServiceImpl.updateCommitteeType(1, committeeTypeDTO).getBody();
