...
class RoleServiceImplTest {
  ...
  @Mock private PrivilegeCategoryRepository privilegeCategoryRepository;

  @Mock private PrivilegeRepository privilegeRepository;

  @InjectMocks private RoleServiceImpl roleService;

  @Mock private RoleTempRepository roleTempRepository;
  ...
  void testFindRolesTempByID_NotFound() {
    ...
    verify(roleTempRepository).findById(roleID);
  }
  ...
  void testSaveRoleAudit_Success() {
    ...
    verify(roleAudRepository).save(roleAudCaptor.capture());
    ...
    verify(rolePrivilegeAudRepository).saveAll(anySet());
  }
  ...
  void testSaveRoleAudit_EmptyPrivileges() {
    ...
    verify(rolePrivilegeAudRepository).saveAll(Collections.emptySet());
  }
  ...
  void testFindAllPrivileges_WhenPrivilegesExist() {
    ...
    List<Privilege> privileges = List.of(new Privilege(1), new Privilege(2));
  ...
  void testGetUserPrivilegesByUMPCode_ConvertsGroupCodeToStringCorrectly() {
    ...
    verify(roleJdbc).getUserPrivilegesByUMPCode(codeCaptor.capture());
  ...
  void testGetUserPrivilegesByUMPCode_WithNegativeGroupCode() {
    ...
    assertNotNull(response);
    assertEquals(HttpStatus.OK, response.getStatusCode());
    verify(roleJdbc).getUserPrivilegesByUMPCode("-1");
  }
